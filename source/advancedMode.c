...
void addToRgb(color_rgb* colorPtr, int selectedColorIndex, int value)
{
	...
	{
		case 0: colorPtr->R += value; break;
		case 1: colorPtr->G += value; break;
		case 2: colorPtr->B += value; break;
	}
}
...
void printAdvancedControls()
{
	...
	printf("\x1b[4;0HUndo last action: \x1b[36mB\x1b[0m");
	printf("\x1b[5;0HAdjust Pen size: \x1b[36mSound Slider\x1b[0m");
}
...
void advancedMode(PrintConsole* topScreenPtr)
{
	...
	consoleClear();
	C2D_TargetClear(bottomPtr, clrClear);

	consoleSelect(topScreenPtr);
	printSimpleControls();
	printAdvancedControls();

	printf("\x1b[7;0HCurrent Mode: ");
	printf("\x1b[8;0HCurrent Pen Size: ");
	printf("\x1b[9;0HCurrent Color");
	...
	while (aptMainLoop())
	{
		...
		if (kDown & KEY_SELECT) C2D_TargetClear(bottomPtr, clrClear);
		...
		printf("\x1b[10;2H\x1b[31m%u  \x1b[0m", color.R);
		printf("\x1b[11;2H\x1b[32m%u  \x1b[0m", color.G);
		printf("\x1b[12;2H\x1b[34m%u  \x1b[0m", color.B);
